...
aggregate all the logs from your {product-title} cluster, such as node system audit logs (logs-audit), application container logs (logs-app), and infrastructure logs (logs-infra).
endif::[]
