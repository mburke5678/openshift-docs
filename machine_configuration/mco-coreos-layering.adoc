...
.Additional resources
For more information on extensions, see xref:../machine_configuration/machine-configs-configure.html#rhcos-add-extensions_machine-configs-configure[Adding extensions to RHCOS]. If you used the procedure in the previous section to install the  extensions, you do not need to perform the steps in the 
