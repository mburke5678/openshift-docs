...
build-layered-c8765e26ebc87e1e17a7d6e0a78e8bae      0/2     Running     0               11m
----
...
<4> Specifies the name of the pull secret that the MCO needs in order to pull the base operating system image from the registry.
...
<6> Specifies the name of the push secret that the MCO needs in order to push the newly-built custom layered image to that registry.
<7> Specifies the secret required by the image registry that the nodes need in order to pull the newly-built custom layered image. This should be a different secret than the one used to push the image to your repository.
// +
