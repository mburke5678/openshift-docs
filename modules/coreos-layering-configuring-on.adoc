...
layered-c8765e26ebc87e1e17a7d6e0a78e8bae   False      False      True        False         False     43
----
...
When the build is complete, the MCO pushes the new custom layered image to your repository and rolls the image out to the nodes in the associated machine config pool. You can see the digested image pull spec for the new custom layered image in the `MachineOSBuild` object and `machine-os-builder` pod.

You should not need to interact with these new objects or the `machine-os-builder` pod. However, you can use all of these resources for troubleshooting, if necessary. Also, you can automatically remove an on-cluster custom layered image from the repository by deleting the `MachineOSBuild` object that created the image. For more information, see "Removing an on-cluster custom layered image". 
...
=======
You need a separate `MachineOSConfig` CR for each machine config pool where you want to use a distinct custom layered image. You can apply a custom layered image by using any machine config pool in your cluster, including the control plane, worker, or custom pool.
>>>>>>> edit
...
----
NAME                                       PREPARED   BUILDING   SUCCEEDED   INTERRUPTED   FAILED   AGE
layered-c8765e26ebc87e1e17a7d6e0a78e8bae   False      False      False        False        True     48s
----

You can manually rebuild your custom layered image by either modifying your `MachineOSConfig` object or applying an annotation to the `MachineOSConfig` object as discussed in "Rebuilding an on-cluster custom layered image".
...
* You have the global pull secret in the `openshift-machine-config-operator` namespace that the MCO needs to pull the base operating system image. You can optionally add your pull secret to the {product-title} global pull secret, because the MCO automatically creates a copy of the global pull secret and uses that by default. For information on modifying the global pull secret, see "Updating the global cluster pull secret".
...
<8> Specifies the image registry to push the newly-built custom layered image to. This can be any registry that your cluster has access to in the the `host[:port][/namespace]/name` or `svc_name.namespace.svc[:port]/repository/name:<tag>` format. This example uses the internal {product-title} registry. You can specify a mirror registry if you cluster is properly configured to use a mirror registry.
...
[source,yaml]
----
Name:         layered-cowsay-710de7ebed5b5b1086015309b2464d8d
Namespace:    
Labels:       machineconfiguration.openshift.io/machine-os-config=layered-cowsay
              machineconfiguration.openshift.io/rendered-machine-config=rendered-layered-e8c8bc1de69777325003e80bc0c04b82
              machineconfiguration.openshift.io/target-machine-config-pool=layered
Annotations:  machineconfiguration.openshift.io/job-uid: d361d56c-ffbd-4e7d-bfb0-8202cbe27592
              machineconfiguration.openshift.io/rendered-image-push-secret: long-lived-token
API Version:  machineconfiguration.openshift.io/v1
Kind:         MachineOSBuild
Metadata:
  Creation Timestamp:  2025-05-02T14:28:55Z
  Finalizers:
    foregroundDeletion
  Generation:  1
  Owner References:
    ...
    Kind:                  MachineOSConfig
    Name:                  layered-cowsay
    UID:                   7977ec05-ba4d-45a1-bc0b-2ef0017c7a14
  Resource Version:        46571
  UID:                     46a6fd2c-697c-4ab3-aba7-2e496c161b0b
Spec:
  Machine Config:
    Name:  rendered-layered-e8c8bc1de69777325003e80bc0c04b82
  Machine OS Config:
    Name:                    layered-cowsay
  Rendered Image Push Spec:  image-registry.openshift-image-registry.svc:5000/openshift-machine-config-operator/os-images:layered-cowsay-710de7ebed5b5b1086015309b2464d8d <1>
Status:
  Build End:    2025-05-02T14:31:01Z
  Build Start:  2025-05-02T14:29:06Z
  Builder:
    Image Builder Type:  Job
    Job:
      Group:      batch
      Name:       build-layered-cowsay-710de7ebed5b5b1086015309b2464d8d
      Namespace:  openshift-machine-config-operator
      Resource:   jobs
  Conditions:
    Last Transition Time:  2025-05-02T14:29:06Z
    ...
    Status:                False
    Type:                  Prepared
    Last Transition Time:  2025-05-02T14:31:01Z
