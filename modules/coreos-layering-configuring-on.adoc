...
You need a separate `MachineOSConfig` CR for each machine config pool where you want to use a distinct custom layered image. You can apply a custom layered image by using any machine config pool in your cluster, including the control plane, worker, or custom pools.
...
* You have the global pull secret in the `openshift-machine-config-operator` namespace that the MCO needs to pull the base operating system image. You can optionally add your pull secret to the {product-title} global pull secret, as the MCO automatically creates a copy of the global pull secret and use that by default. For information on modifying the global pull secret, see "Updating the global cluster pull secret".
