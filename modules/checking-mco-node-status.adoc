...
During the update of a machine config pool (MCP) object, you can monitor the progress of individual nodes in case issues arise and you need to troubleshoot a node.
...
|`TRUE` |The phase is complete.
|`FALSE` |The phase is either being executed, will not be executed, or has errored. If the phase has errored, use the `oc describe machineconfignodes` command for more information.
|`UNKNOWN` |The phase has not started during the most recent upgrade process.
|===

In addition to the fields defined in the previous table, the `-o wide` output displays the following fields:

.Machine config nodes fields in the `-o wide` output
[cols="1,4",options="header"]
|===
|Phase Name |Definition
|`UPDATED` |Indicates if the MCO has applied the current machine config to the node.
...
|`UPDATEPOSTACTIONCOMPLETE` |Indicates whether the MCO has executed its post-update actions on the node.   
|`UPDATECOMPLETE` |Indicates if the MCO has completed the core parts of the update on the node.  
...
|`UPDATEDFILESANDOS` |Indicates if the MCO has updated the nodes file and operating system configuration.
...
To see the status of the Machine Config Operator (MCO) updates to your cluster, use the following `oc` command
...
. View a summary of update statuses for all nodes in the default machine config pools (MCP):
+
* View the current and desired machine configurations by running the following command:
+
[source,terminal]
----
...
* View all machine config node status fields by running the following command:
+
[source,terminal]
----
