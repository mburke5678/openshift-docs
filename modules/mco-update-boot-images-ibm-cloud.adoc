...
. Obtain your {ibm-cloud-title} API key and set the value in an environment variable by running the following command:
+
[source,terminal]
----
...
----
+
For information on creating an {ibm-cloud-title} API key, see link:https://www.ibm.com/docs/en/masv-and-l/cd?topic=cli-creating-your-cloud-api-key[Creating your {ibm-cloud-title} API key in the {ibm-cloud-title}] documentation.
...
. Obtain the URL of the {op-system} image you want to use as the boot image and set the location in an environment variable by running the following commands, based on your cluster architecture:
+
* Linux (x86_64, amd64):
+
[source,terminal]
...
----
+
`RHCOS_URL` is the environment variable you created in previous steps.
...
.. Create a custom image for your {ibm-cloud-title} Virtual Private Cloud (VPC) from the uploaded boot image by running the following command: 
+
[source,terminal]
----
...
+
Use the `descriptive_image_name` parameter to to create a descriptive name for your boot image in the COS bucket. Setting a descriptive name for your custom image, such as using the {op-system-first} version number in the image name, makes it easier to track which version is currently deployed if you update the cluster in the future.
