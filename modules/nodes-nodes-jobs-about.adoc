...
By default, for jobs that are not associated with a cron job or other controlling object, {product-title} does not delete the job or its related pods when the job is finished. Having these artifact remain in the cluster allows you to view the status of completed jobs or view job logs for errors, warnings, or other diagnostic output. 
...
You can configure a time-to-live (TTL) duration for jobs without a controlling object by setting the `ttlSecondsAfterFinished` parameter in the job spec. When set, {product-title} deletes the jobs and any related pods immediately after the job is finished or after a specified period of time.
