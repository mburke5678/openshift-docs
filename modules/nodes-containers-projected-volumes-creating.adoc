...
[role="_abstract"]
You can create a pod spec to use a projected volume that mount several existing volume sources into the same directory. 
...
.. Create a YAML file similar to the following example, replacing the password and user information as appropriate:
+
[source,yaml]
----
apiVersion: v1
...
.. Create a YAML file similar to the following example, including a `volumes` section:
+
[source,yaml]
----
kind: Pod
...
----
+
Replace <mysecret> with the name of the secret you created.
