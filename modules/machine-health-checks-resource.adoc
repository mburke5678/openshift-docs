...
<5> Specify the node conditions to check for, as needed. This example checks all node conditions. See the node conditions in the table below.
<6> Specify the timeout duration for a node condition. If a condition is met for the duration of the timeout, the machine will be remediated. Long timeouts can result in long periods of downtime for a workload on an unhealthy machine.
...
<4> Specify the node conditions to check for, as needed. This example checks all node conditions. See the node conditions in the table below.
