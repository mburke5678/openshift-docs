...
** `spec.config.type` specifies the `cnv-tuning` plugin that provides support for custom MAC addresses.
+
[NOTE]
====
The virtual machine instance is attached to the bridge `br1` through an access port with the requested VLAN.
====
