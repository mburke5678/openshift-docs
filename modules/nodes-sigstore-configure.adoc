...
You can use the `ClusterImagePolicy` and `ImagePolicy` custom resource (CR) objects to enable and configure Sigstore support for the entire cluster or a specific namespace. These objects specify the images and repositories to be verified by using Sigstore tooling and how the signatures must be verified.
...
`scopes`:: Defines a list of respositories and images assigned to a policy. You must list at least one of the following scopes:
+
--
...
* `rootOfTrust`: Specifies the root of trust for the policy. Configure either a public key or a link:https://docs.sigstore.dev/certificate_authority/overview/[Fulcio certification].
...
** `FulcioCAWithRekor`: Indicates that the policy is based on the Fulcio certification. You must specify the following parameters:
