...
[role="_abstract"]
For a Nutanix cluster, you can manually update the boot image for your cluster by configuring your machine sets to use the latest {product-title} image as the boot image to help ensure any new nodes can scale up properly.
...
[source,terminal]
----
$ export PC_IP="<prism_central_ip_adress>"
----
...
----
+
Setting a descriptive name for your boot image in Prim Central, such as using the {op-system-first} version number in the image name, makes it easier to track which version is currently deployed if you update the cluster in the future.
+
.Example command
[source,terminal]
----
...
After you migrate all machine sets to the new boot image, the old boot image is no longer needed. You can perform the following steps to remove the image from Prism Central.

. Find the UUID for the image by running the following command:
+
[source,terminal]
----
curl -k -u "$USER:$PASS" \
...
----
curl -k -u "$USER:$PASS" \
  -X DELETE "https://$PC_IP:9440/api/nutanix/v3/images/<image-uuid>" \
  -H "Content-Type: application/json"
----
+
Replace `image_uuid` with the UUID of the image.
