...
+
After the reboot, SSH into any Windows node on AWS and add the route by running the following command in a command prompt:
+
[source,terminal]
----
C:\> route add <address_mask> mask <network_mask> <gateway_ip>
----
+
where:
+
--
`<address_mask>`:: Specifies the address mask of the EC2 instance metadata endpoint.
`<network_mask>`:: Specifies the network mask of the EC2 instance metadata endpoint.
