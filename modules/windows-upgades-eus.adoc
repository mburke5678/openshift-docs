...
{product-title} and Windows Machine Config Operator (WMCO) support updating from one EUS version to another EUS version of OCP(https://access.redhat.com/support/policy/updates/openshift-eus), in a process called a _Control Plane Only update_. After upgrading the cluster, the Windows nodes will be upgraded from the staring EUS to the new EUS version while keeping the Windows workloads in a healthy state with no disruptions.
...
* All the of the [prerequisites of the Control Plane Only upgrade](https://docs.openshift.com/container-platform/latest/updating/updating_a_cluster/eus-eus-update.html)
