...
With boot image management disabled, you are responsible to update the boot image version used by the boot image skew enforcement feature to verify that the boot image version is up-to-date for your cluster. For more information, see "Boot image skew enforcement".
