...
* If you are disabling boot image management for control plane machine sets, you enabled the required Technology Preview features for your cluster by editing the `FeatureGate` CR named `cluster`.
...
`spec.managedBootImages.machineManagers.selection.resource.machinesets`:: Specifies that the `selection.mode` action applies to compute nodes.
...
`spec.managedBootImages.machineManagers.selection.resource.controlplanemachinesets`:: Specifies that the `selection.mode` action applies to control plane nodes.
