...
To disable the updated boot image feature, edit the `MachineConfiguration` object to have an empty list for the `machineMangers` field.
...
# ...
  managedBootImages: <1>
    machineManagers: []
----
<1> Remove the parameters under `machineManagers` and add the `[]` to disable updated boot images.
