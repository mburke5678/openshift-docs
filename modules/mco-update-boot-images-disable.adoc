...
To disable the updated boot image feature, edit the `MachineConfiguration` object to remove the `managedBootImages` parameters.
...
# ...
  managedBootImages: <1>
    machineManagers: []
----
<1> Remove the parameters under `machineManagers` to disable updated boot images.
